...
import { Button, ButtonGroup, ButtonToolbar, Card, Col, Container, Dropdown, DropdownButton, Form, InputGroup, Nav, Pagination, Row } from 'react-bootstrap';
...
import { fromUnixTime, isBefore, startOfDay, subHours } from 'date-fns';
...
export const SupportTickets: React.FC = () => {
    ...
    const isTicketOverDue = (timestamp: number) => {
    ...
    const getSupportAllTickets = async () => {
        try {
            ...
            const overdue = response.data.filter((ticket: SupportTicketInterface) =>
                ...
                isTicketOverDue(ticket.createdDate)
            );
            ...
            const resolvedToday = response.data.filter((data: { status: string; resolvedDate?: number }) =>
                data.status.toUpperCase() === 'RESOLVED' &&
                data.resolvedDate !== undefined &&
                data.resolvedDate >= todayStart &&
                data.resolvedDate < todayEnd
            );
...
const handleTabsAndFilters = () => {
    ...
    if (statusFilter === 'resolved') {
        ...
        const todayStart = getStartOfToday();
        filtered = filtered.filter(data => data.status.toUpperCase() === 'RESOLVED' && data.resolvedDate! >= todayStart &&
            data.resolvedDate! < todayStart + 86400000);
...
return (
    <>
        <Container className='top-container'>
            ...
            <TicketOverview pendingCounter={pendingCounter} overDueCounter={overDueCounter} resolvedTodayCounter={resolvedTodayCounter} statusFilter={setStatusFilter} />
        </Container>
